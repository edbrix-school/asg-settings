...
public class TermsTemplateController {
    ...
    @Operation(
            summary = "List Terms Templates with Search and Sort",
            description = "Provide search filters. Valid `searchField` values: GLOBALSEARCH or (TEMPLATE_ID, DOC_ID, TEMPLATE_NAME, TERMS_CATEGORY). Sorting default on termsPoid, desc." +
    ...
    @PostMapping("/list")
    public ResponseEntity<?> getTermsTemplateList(
        ...
        try {
            Map<String, Object> companies = termsTemplateService.listTerms(documentId, filters, pageable);
    ...
    @Operation(
            summary = "Update Terms and Clauses",
            description = "Updating an existing Terms and Clauses. This endpoint allows you to update Terms and Clauses." +
                    " The `termsPoid` is required to identify the parameter to be updated. The updated details are provided in the request body." +
                    " For clauses, use actionType: 'isCreated' for new clauses, 'isUpdated' for modifications, 'isDeleted' for soft deletion." +
                    " The response indicates the success or failure of the update operation." +
                    "\n\n### Required Headers: loginUserPoid=3371"
    )
    @io.swagger.v3.oas.annotations.parameters.RequestBody(
            required = true,
            description = "Terms template details with clauses to be updated. Use actionType in clauses: 'isCreated', 'isUpdated', 'isDeleted', 'noChange'",
            content = @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = TermsTemplateDto.class),
                    examples = @ExampleObject(
                            name = "Update Terms Template with Clause Actions Example",
                            value = """
                                    {
                                        "termsPoid": 406,
                                        "groupPoid": 1,
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Successfully updated the terms template",
                    content = @Content(
                            mediaType = "application/json",
    ...
    public ResponseEntity<?> updateTemplateMetadata(@Parameter(description = "termsPoid identifier", example = "208",
            required = true) @PathVariable Long termsPoid,
                                                    @Parameter(description = "loginUserPoid identifier", example = "3371",
                                                            required = true) @RequestHeader("loginUserPoid") String loginUserPoid,
                                                    @Valid @RequestBody TermsTemplateDto request,
                                                    @Parameter(description = "Document identifier", example = "000-015", required = true)
                                                    @RequestParam String documentId,
                                                    @Parameter(description = "Action requested", example = "EDIT", required = true)
                                                    @RequestParam String actionRequested) {
        try {
    ...
    @Operation(
            summary = "Create New Terms Template with Clauses",
            description = "Creates a new terms template along with its associated clauses. " +
                    "The template will be created with the provided details and associated with the specified group. " +
                    "**Note:** templateId is auto-generated by database trigger and should not be provided in the request.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    description = "Terms template details with clauses to be created",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = TermsTemplateDto.class),
                            examples = @ExampleObject(
                                    name = "Create Terms Template Example",
                                    value = """
                                            {
                                                "docId": "400-007",
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Successfully created the terms template",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = TemplateResponseDto.class),
                            examples = @ExampleObject(
    ...
    @PostMapping
    public ResponseEntity<?> createTermsAndConditionsTemplate(@Parameter(description = "groupPoid identifier", example = "1", required = true)
                                                              @RequestHeader("groupPoid") Long groupPoid,
                                                              @Parameter(description = "loginUserPoid identifier", example = "123", required = true)
                                                              @RequestHeader("loginUserPoid") String loginUserPoid,
                                                              @Valid @RequestBody TermsTemplateDto request,
                                                              @Parameter(description = "Document identifier", example = "000-015", required = true)
                                                              @RequestParam String documentId,
                                                              @Parameter(description = "Action requested", example = "CREATE", required = true)
                                                              @RequestParam String actionRequested) {
    ...
    @Operation(summary = "Get Terms Templates And Clauses by termsPoid",
            description = "Retrieves terms template and associated clauses based on the provided termsPoid. " +
                    "The response includes all template details along with its clauses.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Terms Templates and Clauses retrieved successfully",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = TermsTemplateDto.class)
                    )),
            @ApiResponse(responseCode = "404", description = "Terms Templates and Clauses not found"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @GetMapping("/{termsPoid}")
    public ResponseEntity<?> getTermsTemplateAndClauses(@Parameter(
            description = "Unique identifier of the terms template",
            required = true, example = "30")
                                                        @PathVariable Long termsPoid,
    ...
    @Operation(
            summary = "Delete the Terms & Conditions Template By termsPoid",
            description = "Soft Delete the Terms & Conditions By termsPoid if its deleted then delete the associated Clauses"
    )
    @DeleteMapping("/{termsPoid}")
    public ResponseEntity<?> softDeleteTemplate(@Parameter(
            description = "Unique identifier of the terms template",
            required = true, example = "30")
                                                @PathVariable Long termsPoid,
    ...
    public ResponseEntity<?> softDeleteTermsAndClauses(@Parameter(
            description = "Unique identifier of the Terms template",
            example = "30",
            required = true
    )
                                                       ...
                                                       @Parameter(
                                                               description = "Clause number to be soft deleted",
                                                               example = "001",
                                                               required = true
                                                       )
                                                       @PathVariable String clauseNo,
                                                       @Parameter(
                                                               description = "Document identifier",
                                                               example = "350-103",
                                                               required = true)
    ...
    @Operation(
            summary = "Add new Clause to the Given Terms & Conditions Template",
            description = "Add new Clause to the Given Terms & Conditions Template"
    )
    @PostMapping("/{termsPoid}/clause")
    public ResponseEntity<?> addClauseToTemplate(
            @Parameter(
                    description = "Unique identifier of the Terms template",
                    example = "30",
                    required = true
            ...
            @RequestParam String actionRequested,
            @Valid
            @RequestBody TermsTemplateDtlDto dto
    ) {
